...
import { BsTrash, BsBookmarkCheck, BsBookmarkCheckFill, BsHandThumbsDown } from "react-icons/bs";
...
function App() {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      const res = await fetch(API + "/todos").then((res) => res.json()).catch((err) => console.log(err));
      setLoading(false);
      setTodos(res);
    };
    loadData();
